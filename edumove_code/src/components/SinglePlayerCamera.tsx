...
export default function SinglePlayerCamera({ questions, onExit, experimentName }: Props) {
  ...
  const fontStyle = { fontFamily: "'Kanit', sans-serif" };

  if (status === 'SUMMARY') {
    const percentage = Math.round((score / questions.length) * 100);
    let message = "เกณฑ์การประเมิน: ควรทบทวนเนื้อหาเพิ่มเติม";
    let color = "text-rose-200";
    let gradient = "from-purple-400 to-pink-500";
    
    if (percentage >= 80) { 
      message = "เกณฑ์การประเมิน: ระดับดีเยี่ยม (Excellent)"; color = "text-white"; gradient = "from-amber-300 to-orange-500"; 
    }
    else if (percentage >= 50) { 
      message = "เกณฑ์การประเมิน: ผ่านเกณฑ์ (Pass)"; color = "text-cyan-100"; gradient = "from-cyan-400 to-blue-500"; 
    }

    return (
      <div className="w-full h-screen flex flex-col items-center justify-center bg-[#0f172a] relative overflow-hidden" style={fontStyle}>
        <div className="absolute inset-0 bg-[url('/bg-pattern.svg')] opacity-10 pointer-events-none"></div>
        <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-cyan-400/20 rounded-full blur-[100px] pointer-events-none"></div>
        <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-pink-500/20 rounded-full blur-[100px] pointer-events-none"></div>

        <motion.div initial={{ scale: 0.9, opacity: 0, y: 30 }} animate={{ scale: 1, opacity: 1, y: 0 }} className="bg-white/10 backdrop-blur-2xl border-2 border-white/20 p-10 md:p-14 rounded-[3rem] shadow-[0_20px_60px_rgba(0,0,0,0.5)] max-w-2xl w-full text-center relative z-10">
          <motion.div animate={{ y: [-10, 10, -10] }} transition={{ repeat: Infinity, duration: 4 }}>
            <Trophy size={110} className="mx-auto text-amber-400 drop-shadow-[0_10px_20px_rgba(250,204,21,0.5)] mb-6" />
          </motion.div>
          <h1 className="text-3xl md:text-5xl font-black text-white mb-2 tracking-wide text-balance">รายงานผลการประเมิน</h1>
          <p className={`text-xl font-bold mb-8 drop-shadow-md text-balance ${color}`}>{message}</p>
          
          <div className={`bg-gradient-to-br ${gradient} rounded-[2rem] p-8 border-4 border-white/30 shadow-inner mb-8 transform -rotate-1`}>
            <p className="text-white/90 text-lg mb-1 font-bold uppercase tracking-widest">คะแนนที่ทำได้</p>
            <div className="flex items-baseline justify-center gap-2">
              <span className="text-7xl md:text-8xl font-black text-white drop-shadow-lg">{score}</span>
              <span className="text-3xl font-bold text-white/80">/ {questions.length}</span>
            </div>
          </div>

          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button onClick={restartGame} className="flex-1 px-8 py-4 rounded-full font-black text-xl text-slate-900 bg-amber-400 hover:bg-amber-300 flex items-center justify-center gap-2 transition-all shadow-[0_10px_20px_rgba(251,191,36,0.4)] hover:scale-105 active:scale-95">
              <RotateCcw size={24} /> ทำแบบทดสอบอีกครั้ง
            </button>
            <button onClick={() => onExit && onExit(score)} className="flex-1 px-8 py-4 rounded-full font-bold text-lg text-white bg-slate-800 hover:bg-slate-700 border-2 border-slate-600 flex items-center justify-center gap-2 transition-all hover:scale-105">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-30 pointer-events-none flex flex-col">
            ...
            <div className="absolute inset-x-0 bottom-[5%] md:bottom-[8%] flex justify-between items-stretch px-2 sm:px-6 md:px-12 w-full">
              <div className={`w-[45%] max-w-[450px] bg-gradient-to-br from-cyan-500/90 to-blue-600/90 backdrop-blur-xl border-4 rounded-[2rem] p-5 md:p-8 text-center transition-all duration-300 flex flex-col items-center justify-center min-h-[160px] md:min-h-[220px] ${lockedChoice === 'LEFT' ? 'border-white scale-110 shadow-[0_0_60px_rgba(6,182,212,0.8)] z-10' : 'border-white/50 shadow-2xl'} ${(lockedChoice && lockedChoice !== 'LEFT') ? 'opacity-30 scale-90' : ''}`}>
                <span className="text-cyan-100 font-bold mb-3 md:mb-4 text-xs sm:text-sm bg-black/20 px-4 py-1.5 rounded-full uppercase tracking-wider shrink-0">เอียงศีรษะซ้าย</span>
              ...
              <div className={`w-[45%] max-w-[450px] bg-gradient-to-br from-pink-500/90 to-rose-600/90 backdrop-blur-xl border-4 rounded-[2rem] p-5 md:p-8 text-center transition-all duration-300 flex flex-col items-center justify-center min-h-[160px] md:min-h-[220px] ${lockedChoice === 'RIGHT' ? 'border-white scale-110 shadow-[0_0_60px_rgba(244,63,94,0.8)] z-10' : 'border-white/50 shadow-2xl'} ${(lockedChoice && lockedChoice !== 'RIGHT') ? 'opacity-30 scale-90' : ''}`}>
                <span className="text-pink-100 font-bold mb-3 md:mb-4 text-xs sm:text-sm bg-black/20 px-4 py-1.5 rounded-full uppercase tracking-wider shrink-0">เอียงศีรษะขวา</span>
