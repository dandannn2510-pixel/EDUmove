...
import { Play, CheckCircle, XCircle, Timer, Hand, FastForward, Lightbulb, Sparkles } from 'lucide-react';
...
export default function CameraDetection({ 
  ...
  if (status === 'INTRO') {
    return (
      <div className="w-full h-full min-h-[85vh] flex items-center justify-center relative p-4 select-none" style={{ fontFamily: "'Kanit', sans-serif" }}>
        ...
        <div className="w-full max-w-3xl bg-slate-800/90 backdrop-blur-xl border border-slate-600 p-6 sm:p-12 rounded-[2rem] shadow-2xl flex flex-col items-center text-center">
          <h1 className="text-3xl sm:text-5xl font-black text-white mb-2">เตรียมพร้อมเข้าสู่ Pre-test</h1>
          <div className="bg-indigo-500/20 text-indigo-300 font-bold px-4 py-1 rounded-full border border-indigo-500/30 mb-6 text-xs sm:text-sm">โหมด 4 ตัวเลือก (A, B, C, D)</div>
          <div className="w-full bg-slate-900 rounded-2xl p-4 text-left mb-8 border border-slate-700 shadow-inner flex items-start gap-3">
            <div className="bg-emerald-500/20 p-2.5 rounded-xl text-emerald-400 shrink-0"><Hand size={20}/></div>
            <p className="text-xs sm:text-base text-slate-400 leading-relaxed">ชูมือไปยัง <strong className="text-emerald-400">สีของข้อที่ต้องการ</strong> ค้างไว้ 1 วินาทีเพื่อล็อคคำตอบ หากเปลี่ยนใจสามารถย้ายมือได้ก่อนหมดเวลา!</p>
          </div>
          <div className="flex flex-col items-center gap-4 w-full">
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => { setStatus('SHOW_TITLE'); }} className="px-8 sm:px-12 py-3.5 sm:py-4 rounded-full font-black text-lg sm:text-xl text-white bg-indigo-600 shadow-lg flex items-center gap-2"><Play size={24} /> เปิดกล้องเริ่มเกม</motion.button>
            <div className="flex flex-wrap items-center justify-center gap-4 border-t border-slate-700 pt-4 w-full">
              {onViewAnswers && <button onClick={onViewAnswers} className="text-amber-400 font-bold text-xs sm:text-base flex items-center gap-1">ดูเฉลย Pre-test <Lightbulb size={16} /></button>}
              {onSkip && <button onClick={onSkip} className="text-slate-400 font-bold text-xs sm:text-base flex items-center gap-1">ข้ามไปวิดีโอ <FastForward size={16} /></button>}
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-center overflow-hidden select-none" style={{ fontFamily: "'Kanit', sans-serif" }}>
      <style dangerouslySetInnerHTML={{ __html: `@import url('https://fonts.googleapis.com/css2?family=Kanit:wght@400;600;800;900&display=swap');` }} />
      ...
      <AnimatePresence>
        {status === 'SHOW_TITLE' && (
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.2 }} className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-slate-900/90 backdrop-blur-md">
            ...
            <h2 className="text-xl sm:text-4xl text-indigo-400 font-black mb-4 uppercase tracking-widest">โจทย์การทดลองเรื่อง</h2>
            <h1 className="text-4xl sm:text-7xl text-white font-black drop-shadow-2xl text-center px-4 leading-tight text-transparent bg-clip-text bg-gradient-to-r from-amber-200 to-yellow-500">
              {experimentName || 'การทดลองวิทยาศาสตร์'}
            </h1>
          </motion.div>
        )}
      </AnimatePresence>
