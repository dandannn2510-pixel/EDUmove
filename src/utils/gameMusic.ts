...
class KahootMusicSynth {
  ...
  public set isMuted(value: boolean) {
    ...
    if (this.masterGain && this.ctx) {
      this.masterGain.gain.setValueAtTime(value ? 0 : 3.6, this.ctx.currentTime);
    }
  }
  ...
  public initCtx() {
    ...
    if (!this.ctx) {
      ...
      this.masterGain.gain.setValueAtTime(this._isMuted ? 0 : 3.6, this.ctx.currentTime); // Soft background level

      // Create a dynamics compressor if available (to prevent clipping at high volumes)
  ...
  private scheduleNextNote() {
    ...
    // Bass note (made louder from 0.12 to 0.24)
    const bassMidi = this.bassSeq[this.currentStep % this.bassSeq.length];
    if (bassMidi > 0) {
      this.playNote(this.midiToFreq(bassMidi), this.nextNoteTime, stepDuration * 0.9, 'triangle', 0.24);
    }

    // Melody note (made louder: triangle from 0.05 to 0.12, square from 0.015 to 0.04)
    const melodyMidi = this.melodySeq[this.currentStep % this.melodySeq.length];
    if (melodyMidi > 0) {
      // Play a soft triangle melody note
      this.playNote(this.midiToFreq(melodyMidi), this.nextNoteTime, stepDuration * 0.7, 'triangle', 0.12);
      // Play a very quiet square note to give it a nice 8-bit vibe
      this.playNote(this.midiToFreq(melodyMidi), this.nextNoteTime, stepDuration * 0.3, 'square', 0.04);
    }

    // Percussive clock tick on every beat (steps 0, 2, 4, 6...) (made louder from 0.02 to 0.04)
    if (this.currentStep % 2 === 0) {
      this.playTick(this.nextNoteTime, 0.04);
    }
  ...
  public playIncorrectSound() {
    ...
    gainNode.gain.linearRampToValueAtTime(0.60, now + 0.05); // Increased from 0.25
    ...
    gainNode2.gain.linearRampToValueAtTime(0.20, now + 0.05); // Increased from 0.08
  ...
  public playTimeoutSound() {
    ...
    notes.forEach((note, index) => {
      ...
      gainNode.gain.linearRampToValueAtTime(0.50, now + index * 0.15 + 0.02); // Increased from 0.12
  ...
  public playCountdownTickSound(isGo: boolean = false) {
    ...
    gainNode.gain.setValueAtTime(0, now);
    gainNode.gain.linearRampToValueAtTime(isGo ? 0.60 : 0.50, now + 0.01); // Increased to be louder than music
  ...
  public playStageClearSound() {
    ...
    notes.forEach((note, index) => {
      ...
      gainNode.gain.linearRampToValueAtTime(0.60, now + index * 0.06 + 0.02); // Increased from 0.2
    ...
    chord.forEach((note) => {
      ...
      gainNode.gain.linearRampToValueAtTime(0.35, now + 0.6 + 0.05); // Increased from 0.12
  ...
  public playStageFailSound() {
    ...
    notes.forEach((note, index) => {
      ...
      gainNode.gain.linearRampToValueAtTime(0.40, now + index * 0.12 + 0.02); // Increased from 0.1
